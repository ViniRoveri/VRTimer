...
const PopupTempo = (props:{display:string,setDisplay:Dispatch<SetStateAction<string>>,copiaLocalStorage:Array<Tempo>,        setCopiaLocalStorage:Dispatch<SetStateAction<Tempo[]>>,tempoSelecionado:Tempo|undefined,indexTempoSelecionado:number|undefined,mudouTempo:boolean,setMudouTempo:Dispatch<SetStateAction<boolean>>,displayExcluir:string,setDisplayExcluir:Dispatch<SetStateAction<string>>,botaoAtivoCopiar:string,setBotaoAtivoCopiar:Dispatch<SetStateAction<string>>})=>{
    ...
    return(
        ...
        <section className="popupTempo" style={{animation: 'animaPopup .2s ease-out forwards',display:props.display}}>
            ...
            <button className={`texto botaoCopiar ${props.botaoAtivoCopiar}`} onClick={()=>{
                navigator.clipboard.writeText
                (`Time: ${props.tempoSelecionado?.foiDnf ? 'DNF' : numeroTempo}\r\nScramble: ${props.tempoSelecionado?.scramble}\r\nDate: ${data.getDate()} / ${data.getMonth()+1} / ${data.getFullYear()} - ${String(data.getHours()).padStart(2,'0')}:${String(data.getMinutes()).padStart(2,'0')}:${String(data.getSeconds()).padStart(2,'0')}\r\nPenalties: ${props.tempoSelecionado?.foiMais2?'+2':'None'}`)
                props.setBotaoAtivoCopiar('botaoAtivo')
            }}>{props.botaoAtivoCopiar?'Copied':'Copy'}</button>
            <h5 className="texto">{props.tempoSelecionado?.scramble}</h5>
            <h6 className="texto">{`${data.getDate()} / ${data.getMonth()+1} / ${data.getFullYear()} - ${String(data.getHours()).padStart(2,'0')}:${String(data.getMinutes()).padStart(2,'0')}:${String(data.getSeconds()).padStart(2,'0')}`}</h6>
            <div>
        ...
        <PopupExcluirTempo display={props.displayExcluir} setDisplay={props.setDisplayExcluir} copiaLocalStorage={props.copiaLocalStorage} setCopiaLocalStorage={props.setCopiaLocalStorage} setDisplayPai={props.setDisplay} indexTempoSelecionado={props.indexTempoSelecionado} setBotaoAtivoCopiar={props.setBotaoAtivoCopiar}>{`Delete time ${props.tempoSelecionado?.foiDnf ? 'DNF' : numeroTempo}?`}</PopupExcluirTempo>
        </>
    )
}
