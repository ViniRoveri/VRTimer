...
const Tempos = (props:{copiaLocalStorage:Array<Tempo>,setCopiaLocalStorage:Dispatch<SetStateAction<Tempo[]>>,aberturaMenu:string,mudouTempo:boolean,setMudouTempo:Dispatch<SetStateAction<boolean>>})=>{
    ...
    return(
        <div className="tempos">
            <PopupExcluirTodos display={displayExcluirTodos} setDisplay={setDisplayExcluirTodos} copiaLocalStorage={props.copiaLocalStorage} setCopiaLocalStorage={props.setCopiaLocalStorage}>Delete all times?</PopupExcluirTodos>
            <span>
                <h2 className="subtitulo">Times</h2>
                <p className={`texto ${props.copiaLocalStorage.length>0?'':'invisivel'}`}>{`Number of times: ${props.copiaLocalStorage.length}`}</p>
            ...
            <div style={{height:alturaDiv}}>
                ...
                <span className={`${naoHaTempos} texto`}>There Are No Times Yet...</span>
            </div>
        </div>
    )
}
